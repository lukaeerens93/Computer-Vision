...
for k,v in params.items():
    ...
    size_v = len(v.flatten())
    for index in range(size_v):
        i = np.unravel_index(index,v.shape)

        # Copy this shit from above
        params[k][i] = params[k][i] + eps
        layer1 = forward(     x, params, 'layer1')
        output = forward(layer1, params, 'output', softmax)
        loss_a, _ = compute_loss_and_acc(y, output)

        params[k][i] = params[k][i] - 2*eps
        layer1 = forward(     x, params, 'layer1')
        output = forward(layer1, params, 'output', softmax)
        loss_s, _ = compute_loss_and_acc(y, output)
        params[k][i] = params[k][i] + eps

        central_diff = (loss_a - loss_s) / (2*eps)
        params['grad_'+k][i] = central_diff

        #print("loss_a: " + str(loss_a) + " , loss_s: " + str(loss_s) +" , central_diff: " + str(central_diff) )
